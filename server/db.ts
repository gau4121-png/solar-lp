...
/**
 * Insert a new contact inquiry and return the inserted ID.
 */
...
/**
 * Insert a file attachment record linked to a contact inquiry.
 */
export async function createContactAttachment(attachment: InsertContactAttachment): Promise<void> {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }

  await db.insert(contactAttachments).values(attachment);
}
...
/**
 * Get attachments for a specific inquiry.
 */
